...
//thread methods must strictly follow the standards 
//of library and related prototypes 
//
//this is a typical, thread method
//arg is a pointer passed, using param4 of pthread_create() 
//arg can be NULL 
//as per the prototype, we can return pointer to some object, or
//we can just return NULL
//
//how do we pass void *arg to a thread-method ??
//-->we pass a parameter to p4 of pthread_create() 
//-->this will be stored in a hw-context area of this
//   thread, during thread-creation  
//
//how is it actually passed to this thread-method ??
//
//when this thread is dispatched, this param will be passed 
//to the thread-method, as part of hw-context loading 
//
//
void* test_thread3(void *arg)
{
    ...
    while(1); //we can add another job, in this block 
    pthread_exit(NULL); //as per rules, we must use pthread_exit()
                        //do not just use return, or exit(0) 
...
int main()
{
...
 //param1 will be address of an abstract, thread object
   //
   //param2 is used to set the attributes - we will see later
   //if param2 is set to NULL, default attributes are used by the 
   //library and the system!!!

   //param3 is the address of the method associated with 
   //this thread - this will be managed by the library
   //and also passed to a system call API used for thread-creation !!!
   ...
   //thread's method, indirectly - developer is free to pass NULL, if
   //not needed - 
   //this pointer must point to a global memory area in the
   //processi - it can a global static / a global dynamic,
   //not a stack area variable!!! 
   //if pthread_create() has successfully created a new threadj,
   //internally, tdj will be created and other formalities will
 ...
 //threads are created, as per application's requirements ???
   ...
   //by this thread - in this case, the current thread is the 
   //main thread - first param is lib thread id - pthread_join()
   ...
   //by the target thread, when it terminated - in this scenario, 
   //we are passing NULL, since we are not collecting any return 
   //value 
   ...
   //completes - hw and sw clean-up/shutdown  
