...
//in this case, the user-space pipe-object and buffer will be resident 
//, in user-space - any locking or synchronization needs to 
//be done, in user-space /user-space code, explicitly  - 
//in user-space pipe, as well as system-space pipe, 
//the ipc-buffer is used 
//to store and forward data and  is a circular buffer - you will understand
...
//since this IPC set-up is, in user-space, most of the 
//operations are done, in user-space and there are 
//very few system-call APIs - so, the overheads will 
//be minimal, compared to system-space IPC-mechanism 
//
//--->visualize the set-up and faster access ???
//
struct shm_area {
  ...
  //char my_straread[50][256];   //for practice, in assignment-4
};
...
int main()
{
  ...
  //in this design/code/assignment4, 3 semaphores are used, in a semaphore object 
  //the usage of the semaphores are as below - it is done, 
  //as per applications requirements :

  // first semaphore(index 0) will be used for counting free slots/resources
  // in the IPC buffer of user-space pipe  - 
  // this is a resource "counting semaphore type" - decided, by developer - 
  // as discussed in 11_ipcs_2.txt,for more details - this is a 
  // scenario, for using a counting semaphore  
  //
  // second semaphore(index 1) is used , to  lock/provide mutual exclusion 
  // , to the user-space pipe IPC object/buffer set-up -    
  // refer to 11_ipcs_2.txt - this is a binary semaphore -  
  // decided, by the developer !!! this semaphore is responsible 
  // for locking, in this scenario !!!

  //
  //
  // third semaphore is used as another resource "counting semaphore", 
  // for counting filled slots 
  // in the user-space, pipe IPC buffer - this is again a counting semaphore -
  // refer to 11_ipcs_2.txt - another scenario, for a counting semaphore 
  //
  //in this example, all the semaphore instances are 
  //initialized, using a single semctl() call/SETALL command and 
  //an array is passed, with the respective initial values
  //in this context, initial value of first semaphore, index 0 is set to 50
  //--->why the first semaphore's initial value is set to 50 ??
  //in this context, initial value of second semaphore, index 1 is set to 1
  //--->why the second semaphore's initial value is set to 1 ??
  //in this context, initial value of third semaphore, index 2 is set to 0
  //--->why the third semaphore's initial value is set to 0 ??
  // 
  unsigned short ary1[3],ary[] = { 50,1,0};  
  unsigned short ary2[4],ary3[] = { 50,1,0,0}; //wrong set-up...  

  union semun u1;

  struct shm_area *shma ; //a shared memory pointer of 
                          //our IPC data object type 
  ...
  //as always, create a new shared-memory object, 
  ...
  //the new semaphore object can be accessed, using 
  //sem_id2 and the semaphore indices will be 0, 1, and 2..only
  //first semaphore is assigned index  0
  //second semaphore is assigned index 1
  //third semaphore is assigned index  2
  ...
  //appropriately !!!
  //
  //as always attach and set-up a shared-memory segment
  //and place our IPC data object, in the shared-memory segment... 
 ...
 //object !!! 
  ...
  //application's/project requirements 
  shma->rd_index = 0; 
  shma->wr_index = 0;
  ...
  shma->used_slot_count = 0; 
   ...
   while(1)
   {
    ...
    //interface/an active file - effectively, reading from the terminal 
