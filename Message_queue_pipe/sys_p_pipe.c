...
int main(int argc, char *argv[])
{
   ...
   //pipe() system call  copies the values of handles
   //into pfd[0] and pfd[1] - pfd[0]'s value  will be a read handle/index
   //and pfd[1]'s value will be a write handle/index..
   //
   //we can pass these handles to other system-call APIs...
   //
 ...
 while(i++<2)
 {
   ...
   if(ret==0 && i==1){
	   ...
	   if(ret1<0){ } 
	   close(pfd[0]); //close read-handle, since job is done 
   }
 ...
 //while loop
   ...
   if(ret==0 && i==2)
   {
      ...
      //write(pfd[1],wr_buf,strlen(wr_buf)+1); //this is right
                                             ...
                                             //using write handle  
      close(pfd[1]); //close write-handle, since job is complete 
   }

  } //while loop 
 
   //parent-block of this application
   ...
   if(ret>0){
       while(1){
          ...
          //need to add more code for error-handling ??
